#!/usr/bin/env python3
"""Seed Batch 10 Masonry DIY guide into content/guides + public/images/guides."""
...
from batch10_masonry_guide_content import GUIDES, TITLE_SUBJECTS
...
BATCH10_ASSETS = ROOT / "blog-batch-10" / "assets"
GENERATED = BATCH10_ASSETS / "generated"
PROMPTS_DIR = BATCH10_ASSETS / "prompts"
...
def prepare_images(g: dict, *, use_generated: bool = True) -> None:
    slug = g["slug"]
    subject = TITLE_SUBJECTS.get(slug, slug.replace("-", " "))
    backup_root = BATCH10_ASSETS / "pre-brand-backup"
    ...
    if use_generated and (GENERATED / slug).exists() and any((GENERATED / slug).iterdir()):
        ...
        draw_tip(
            IMG / f"{slug}-tip.webp",
            "Dry before you seal",
            g.get("prevention_caption", "Rake failed grout before sealing over moisture."),
        )
...
def seed_one(slug: str, *, content_only: bool = False, skip_brand: bool = False) -> None:
    ...
    if not guide:
        raise SystemExit(f"Unknown Batch 10 slug: {slug}")
    ...
    if not content_only:
        ...
        if not skip_brand:
            n = apply_brand_to_guide(
                slug, IMG, backup_dir=BATCH10_ASSETS / "pre-brand-backup"
            )
