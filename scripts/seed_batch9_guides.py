#!/usr/bin/env python3
"""Seed Batch 9 Carpentry DIY guide into content/guides + public/images/guides."""
...
from batch9_carpentry_guide_content import GUIDES, TITLE_SUBJECTS
...
BATCH9_ASSETS = ROOT / "blog-batch-9" / "assets"
GENERATED = BATCH9_ASSETS / "generated"
PROMPTS_DIR = BATCH9_ASSETS / "prompts"
...
def prepare_images(g: dict, *, use_generated: bool = True) -> None:
    slug = g["slug"]
    subject = TITLE_SUBJECTS.get(slug, slug.replace("-", " "))
    backup_root = BATCH9_ASSETS / "pre-brand-backup"
    guide_backup = backup_root / slug

    if use_generated and (GENERATED / slug).exists() and any((GENERATED / slug).iterdir()):
        # Drop stale placeholder backups so branding uses the new art.
        ...
        draw_tip(
            IMG / f"{slug}-tip.webp",
            "Mark before you nail",
            g.get("prevention_caption", "Find the loose board before fixing at random."),
        )
...
def seed_one(slug: str, *, content_only: bool = False, skip_brand: bool = False) -> None:
    ...
    if not content_only:
        ...
        if not skip_brand:
            n = apply_brand_to_guide(
                slug, IMG, backup_dir=BATCH9_ASSETS / "pre-brand-backup"
            )
