#!/usr/bin/env python3
"""Seed Batch 6 carpentry DIY guides into content/guides + public/images/guides."""
...
from batch6_carpentry_guide_content import GUIDES, TITLE_SUBJECTS
...
BATCH6_ASSETS = ROOT / "blog-batch-6" / "assets"
GENERATED = BATCH6_ASSETS / "generated"
PROMPTS_DIR = BATCH6_ASSETS / "prompts"
...
def prepare_images(g: dict, *, use_generated: bool = True) -> None:
    ...
    if use_generated and (GENERATED / slug).exists() and any((GENERATED / slug).iterdir()):
        ...
        draw_tip(
            IMG / f"{slug}-tip.webp",
            "Fix small play early",
            g.get("prevention_caption", "Tighten loose woodwork before it becomes a rebuild."),
        )
...
def seed_one(slug: str, *, content_only: bool = False, skip_brand: bool = False) -> None:
    ...
    if not content_only:
        ...
        if not skip_brand:
            n = apply_brand_to_guide(slug, IMG, backup_dir=BATCH6_ASSETS / "pre-brand-backup")
