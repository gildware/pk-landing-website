#!/usr/bin/env python3
"""Seed Batch 12 Pest Control DIY guide into content/guides + public/images/guides."""
...
from batch12_pest_control_guide_content import GUIDES, TITLE_SUBJECTS
...
BATCH12_ASSETS = ROOT / "blog-batch-12" / "assets"
GENERATED = BATCH12_ASSETS / "generated"
PROMPTS_DIR = BATCH12_ASSETS / "prompts"
...
def prepare_images(g: dict, *, use_generated: bool = True) -> None:
    slug = g["slug"]
    subject = TITLE_SUBJECTS.get(slug, slug.replace("-", " "))
    backup_root = BATCH12_ASSETS / "pre-brand-backup"
    ...
    if use_generated and (GENERATED / slug).exists() and any((GENERATED / slug).iterdir()):
        ...
        draw_tip(
            IMG / f"{slug}-tip.webp",
            "Seal food overnight",
            g.get("prevention_caption", "No open sugar — ants have nothing to march toward."),
        )
...
def seed_one(slug: str, *, content_only: bool = False, skip_brand: bool = False) -> None:
    ...
    if not guide:
        raise SystemExit(f"Unknown Batch 12 slug: {slug}")
    ...
    if not content_only:
        ...
        if not skip_brand:
            n = apply_brand_to_guide(
                slug, IMG, backup_dir=BATCH12_ASSETS / "pre-brand-backup"
            )
