#!/usr/bin/env python3
"""Seed Batch 11 Electrical DIY guide into content/guides + public/images/guides."""
...
from batch11_electrician_guide_content import GUIDES, TITLE_SUBJECTS
...
BATCH11_ASSETS = ROOT / "blog-batch-11" / "assets"
GENERATED = BATCH11_ASSETS / "generated"
PROMPTS_DIR = BATCH11_ASSETS / "prompts"
...
def prepare_images(g: dict, *, use_generated: bool = True) -> None:
    slug = g["slug"]
    subject = TITLE_SUBJECTS.get(slug, slug.replace("-", " "))
    backup_root = BATCH11_ASSETS / "pre-brand-backup"
    ...
    if use_generated and (GENERATED / slug).exists() and any((GENERATED / slug).iterdir()):
        ...
        draw_tip(
            IMG / f"{slug}-tip.webp",
            "Split heavy loads",
            g.get("prevention_caption", "Separate geysers from light circuits before winter."),
        )
...
def seed_one(slug: str, *, content_only: bool = False, skip_brand: bool = False) -> None:
    ...
    if not guide:
        raise SystemExit(f"Unknown Batch 11 slug: {slug}")
    ...
    if not content_only:
        ...
        if not skip_brand:
            n = apply_brand_to_guide(
                slug, IMG, backup_dir=BATCH11_ASSETS / "pre-brand-backup"
            )
