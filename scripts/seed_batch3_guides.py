#!/usr/bin/env python3
"""Seed Batch 3 guides into content/guides + public/images/guides."""
...
from batch3_guide_content import GUIDES, TITLE_SUBJECTS
...
GENERATED = BATCH3_ASSETS / "generated"
PROMPTS_DIR = BATCH3_ASSETS / "prompts"
...
def import_generated_images(slug: str) -> int:
    """Copy PNG/WebP from blog-batch-3/assets/generated/{slug}/ into public guide images."""
...
def prepare_images(g: dict, *, use_generated: bool = True) -> None:
    ...
    if use_generated and (GENERATED / slug).exists() and any((GENERATED / slug).iterdir()):
        ...
        steps = [m[0] for m in g.get("methods", [])][:5] or ["Make it safe", "Check power", "Test settings", "Look for leaks", "Book help"]
        draw_methods_ladder(IMG / f"{slug}-methods.webp", steps)
        draw_diagram(IMG / f"{slug}-diagram.webp", "Diagnosis map", ["What you notice", "Likely cause", "First move"])
        draw_tip(IMG / f"{slug}-tip.webp", "Descale before winter", g.get("prevention_caption", "Annual descaling keeps hard-water scaling from eating heating time."))
...
def seed_one(slug: str, *, content_only: bool = False, skip_brand: bool = False) -> None:
    ...
    if not content_only:
        ...
        if not skip_brand:
            n = apply_brand_to_guide(slug, IMG, backup_dir=BATCH3_ASSETS / "pre-brand-backup")
