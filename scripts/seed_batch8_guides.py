#!/usr/bin/env python3
"""Seed Batch 8 Cleaning DIY guide into content/guides + public/images/guides."""
...
from batch8_cleaning_guide_content import GUIDES, TITLE_SUBJECTS
...
BATCH8_ASSETS = ROOT / "blog-batch-8" / "assets"
GENERATED = BATCH8_ASSETS / "generated"
PROMPTS_DIR = BATCH8_ASSETS / "prompts"
...
def prepare_images(g: dict, *, use_generated: bool = True) -> None:
    ...
    if use_generated and (GENERATED / slug).exists() and any((GENERATED / slug).iterdir()):
        ...
        draw_tip(
            IMG / f"{slug}-tip.webp",
            "Dry first, wet last",
            g.get("prevention_caption", "Vacuum unsettled dust before any mopping."),
        )
...
def seed_one(slug: str, *, content_only: bool = False, skip_brand: bool = False) -> None:
    ...
    if not content_only:
        ...
        if not skip_brand:
            n = apply_brand_to_guide(slug, IMG, backup_dir=BATCH8_ASSETS / "pre-brand-backup")
